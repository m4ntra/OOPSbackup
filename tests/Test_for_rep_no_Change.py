...
def main():
  for rep_no in range (1, 50):
   for lambdadelta in range (-50, 50):
    calcdp(500+5*lambdadelta,rep_no)
   print("one loop")
   print(rep_no, lambdadelta)
  print("done")
  return


def calcdp(wavinput,rep_no):
	rep_no = rep_no
	wav = wavinput
	lam = 400
	thetain = -60/180*pl.pi
	nAir = 1.0
	nBac = 1.38
	nMed = 1.34
	...
	for i in range(len(mR)):
	 if mR[i] == 0:
	  title = "reflect_mode_n_equals_{:d}_rep_no_{:d}.csv".format(mR[i],rep_no)
	  txtdata = open(title,"a")
	  txtdata.write("{:d}, {:f}, {:.8f}\n".format(wavinput,rep_no,Rnew[i]))
	 if mR[i] == -1:
	  title = "reflect_mode_n_equals_{:d}_rep_no_{:d}.csv".format(mR[i],rep_no)
	  txtdata = open(title,"a")
	  txtdata.write("{:d}, {:f}, {:.8f}\n".format(wavinput,rep_no,Rnew[i]))
